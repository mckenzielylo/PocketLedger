...
@section('title', 'Record Payment - ' . $debt->name)
...
<div class="min-h-screen bg-gray-50 dark:bg-gray-900">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-gray-600 dark:text-gray-400">
                        Add a new payment for: <span class="font-semibold">{{ $debt->name }}</span>
                    </p>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-8">
            <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-4">Debt Summary</h3>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    <h4 class="text-sm font-medium text-gray-500 dark:text-gray-400">Total Debt</h4>
                    <p class="mt-1 text-2xl font-semibold text-gray-900 dark:text-white">
                        ${{ number_format($debt->amount, 2) }}
                    </p>
                </div>
                <div>
                    <h4 class="text-sm font-medium text-gray-500 dark:text-gray-400">Total Paid</h4>
                    <p class="mt-1 text-2xl font-semibold text-green-600">
                        ${{ number_format($debt->totalPaid, 2) }}
                    </p>
                </div>
                <div>
                    <h4 class="text-sm font-medium text-gray-500 dark:text-gray-400">Remaining</h4>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
            ...
            <form action="{{ route('debt-payments.store', $debt) }}" method="POST" class="p-6">
                @csrf
                ...
                <div class="mb-6">
                    ...
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                        Maximum payment: ${{ number_format($debt->remainingAmount, 2) }}
                    </p>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    amountInput.addEventListener('input', function() {
        ...
        if (value > maxAmount) {
            this.setCustomValidity('Payment amount cannot exceed remaining debt amount');
        } else {
