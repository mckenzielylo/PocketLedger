    ...
    <div class="bg-white dark:bg-gray-800 shadow">
        <div class="px-4 py-6 sm:px-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">New Debt</h1>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Add a new debt or loan record</p>
                </div>
    ...
    <div class="px-4 py-6 sm:px-6">
        <div class="max-w-2xl mx-auto">
            <div class="bg-white dark:bg-gray-800 shadow rounded-lg">
                <form action="{{ route('debts.store') }}" method="POST" class="p-6 space-y-6">
                    @csrf
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <label class="relative flex cursor-pointer rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 p-4 shadow-sm focus:outline-none debt-type-option" data-type="borrowed">
                                <input type="radio" name="type" value="borrowed" class="sr-only" {{ old('type') === 'borrowed' ? 'checked' : '' }} required>
                            ...
                            <label class="relative flex cursor-pointer rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 p-4 shadow-sm focus:outline-none debt-type-option" data-type="lent">
                                <input type="radio" name="type" value="lent" class="sr-only" {{ old('type') === 'lent' ? 'checked' : '' }}>
                                <div class="flex items-center">
                                    <div class="flex-shrink-0">
                                        <div class="w-6 h-6 rounded-full border-2 border-gray-300 dark:border-gray-600 flex items-center justify-center">
                                            <div class="w-3 h-3 rounded-full bg-primary-600 hidden debt-type-dot"></div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="number" name="interest_rate" id="interest_rate" value="{{ old('interest_rate') }}" step="0.01" min="0" max="100"
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3"
                            class="block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                            placeholder="Additional details about this debt...">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex items-center">
                        ...
                        <label for="is_paid" class="ml-2 block text-sm text-gray-900 dark:text-white">
                    ...
                    <div class="flex justify-end space-x-3 pt-4">
                        <a href="{{ route('debts.index') }}" class="px-4 py-2 border border-gray-300 dark:border-gray-600 text-sm font-medium rounded-lg text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500">Cancel</a>
                        <button type="submit" class="px-4 py-2 border border-transparent text-sm font-medium rounded-lg shadow-sm text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500">Create Debt</button>
                    </div>
                </form>
            </div>
        </div>
